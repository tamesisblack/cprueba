            ...
            success:function( respuesta ){
                if(respuesta){
                    for (let index = 0; index < respuesta.items.length; index++) {
                        ...
                        servicio_lista = '<td>'+ item.partes.description +'</td>';
                        servicio_manual = '<td>'+ item.work_text+'</td>';
                        horas = '<td>'+ item.work_hours+'</td>';
                        vvts = '<td>'+ item.work_price+'</td>';
						parcial = '<td  >'+ item.work_price *item.work_hours +'</td>';
                        action = '<td><button class="btn btn-danger delete_item">Eliminar</button></td>'
                        data = 'data-id='+item.id;
                        $( "#tabla_items > tbody").append('<tr '+data+'>'+categoria+tipo+servicio_lista+horas+vvts+parcial+action+'</tr>');
                    }
